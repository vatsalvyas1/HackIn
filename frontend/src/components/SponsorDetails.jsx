...
const SponsorDetails = () => {
  ...
  if (loading) return <div className="text-center py-8 text-neutral-300 font-mono">Loading sponsor details...</div>;
  if (error) return <div className="text-center py-8 text-red-400 font-mono">Error: {error}</div>;
  if (!sponsor) return <div className="text-center py-8 text-neutral-300 font-mono">Sponsor not found</div>;
          ...
          <div className="flex-grow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-4 py-1 rounded-full text-sm font-medium ${
                sponsor.tier === 'Platinum' ? 'bg-purple-700/50 text-purple-200' :
                sponsor.tier === 'Gold' ? 'bg-yellow-700/50 text-yellow-200' :
